                                        ...
                                        <table align="center" width="550" cellpadding="10" cellspacing="10">
                                            <tr>
                                                <td>
                                                    ...
                                                    <p style="{{ $style['paragraph-black'] }}">An absence request has been submitted and requires your review. The details are below.</p>
                                                    ...
                                                    <p style="{{ $style['paragraph-black'] }}">Type: {{ $leave_request->type == 2 ? 'Personal' : 'Medical' }}</p>

                                                   <p style="{{ $style['paragraph-black'] }}">From: {{ $leave_request->start_date->format('d.m.Y')}}</p>
                                                   <p style="{{ $style['paragraph-black'] }}">To: {{ $leave_request->end_date->format('d.m.Y')}}</p>
                                                   <p style="{{ $style['paragraph-black'] }}">Reason: {{ $leave_request->message }}</p>
                                                    
                                                   <p style="{{ $style['paragraph-black'] }}">Please log in to the admin panel to approve or reject the request. If approved, any affected coursework and deadlines will be adjusted accordingly and the parent will be notified. If the request is rejected, please provide a clear and specific reason so the family fully understands the decision and is not left with unanswered questions.</p>

                                                    <p style="{{ $style['paragraph-black'] }}">Handling absence requests promptly and with clear communication is an important part of maintaining trust with our families. Your thoughtful review of each case is valued.</p>

                                                    <p style="{{ $style['paragraph-black'] }}">→ Review Request: <a href="{{ route('login') }}">{{ route('login') }}</a></p>

                                                   <p style="{{ $style['paragraph-black'] }}"> Thank you for your attention to this.</p>
