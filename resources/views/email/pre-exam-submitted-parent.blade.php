                                        ...
                                        <table align="center" width="550" cellpadding="10" cellspacing="10">
                                            <tr>
                                                <td>
                                                    ...
                                                    <p style="{{ $style['paragraph-black'] }}">We are pleased to let you know that {{$exam->student->name}} has completed the pre-exam for {{$exam->course->course->title}}. This is an important milestone in the preparation process, and the results are now available in the parent portal.</p>

                                                    <p style="{{ $style['paragraph-black'] }}">The purpose of the pre-exam is to give {{$exam->student->name}} — and you, as a parent — a clear and honest picture of where things stand before the final exam. It is not a graded assessment in the traditional sense; it is a diagnostic tool designed to identify areas of strength to build on, and areas where targeted preparation will make the biggest difference to the final result.</p>

                                                    <p style="{{ $style['paragraph-black'] }}">We encourage you to take some time to go through the pre-exam results together with {{$exam->student->name}}. The feedback is specific and actionable, and the window between now and the final exam is exactly when it is most valuable to use it. A focused preparation plan based on these results can meaningfully change the outcome.</p>

                                                    <p style="{{ $style['paragraph-black'] }}">→ View Pre-Exam Results: <a href="{{ route('login') }}">{{ route('login') }}</a>

                                                    <p style="{{ $style['paragraph-black'] }}">If you have any questions about the results or would like guidance on how best to support {{$exam->student->name}}'s preparation from here, our Help Desk team is available and happy to assist..</p>

                                                    <p style="{{ $style['paragraph-black'] }}">{{$exam->student->name}} is on the right track. We look forward to supporting them through to the final exam.</p>
