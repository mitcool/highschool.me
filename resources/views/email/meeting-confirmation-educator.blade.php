                                            ...
                                            <tr>
                                                <td>
                                                    <h1 style="{{ $style['header-1'] }}">
                                                        ...
                                                        <p style="{{ $style['paragraph-black'] }}">We are glad to let you know that you have been assigned to an upcoming meeting with {{$student->fullname()}}. Please review the details below and make sure this is confirmed in your calendar well in advance.</p>

                                                        <p style="{{ $style['paragraph-black'] }}">
                                                            Date: {{$meeting->start->format('d.m.Y')}}
                                                            Time: {{$meeting->start->format('H:i')}}
                                                            Meeting type: {{$meeting->curriculum_type->name}}
                                                            Duration: 45 minutes
                                                        </p>

                                                        <p style="{{ $style['paragraph-black'] }}">Please log in to the educator portal to confirm the assignment and access the meeting link. If anything about the scheduling or session format requires clarification before the meeting takes place, please coordinate with the administration team through the portal.</p>

                                                        <p style="{{ $style['paragraph-black'] }}">These sessions are a meaningful part of the personal support we provide to our students. The time and care you bring to each one makes a genuine difference to the experience students have here, and we are grateful for your commitment..</p>

                                                        <p style="{{ $style['paragraph-black'] }}">If for any reason you are unable to attend at the assigned time, please notify the administration team as early as possible so alternative arrangements can be made without affecting the student.</p>

                                                        <p style="{{ $style['paragraph-black'] }}">→ Go to Educator Portal: <a href="{{ route('login') }}">{{ route('login') }}</a></p>

                                                        <p style="{{ $style['paragraph-black'] }}">We look forward to hearing that the meeting went well.</p>
