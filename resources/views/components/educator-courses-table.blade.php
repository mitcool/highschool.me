...
<div class="container my-5">
    ...
    <h2 class="text-center mb-4">Apply </h2>
                                               ...
                                               {{-- Without Transfer Program Courses --}}
                                                @if($cc->curriculum_type_id != 11) 
                                                    ...
                                                    <div class="course-row d-flex justify-content-between align-items-center">
                                                       ...
                                                       @if(in_array($cc->id,$educator_courses))
                                                            <span class="btn btn-completed">✓ Active Course</span> 
                                                       @elseif(in_array($cc->id,$pending_courses))
                                                             <span class="btn btn-in-progress">Pending Approval</span> 
                                                       @else
                                                        <form class="confirm-first enroll-form" action="{{ route('apply-course',$cc->id) }}" method="POST" id="enroll-form-{{ $cc->id }}">
                                                                {{ csrf_field() }}
                                                                <button class="btn btn-enroll"> Apply</button>
                                                            </form>
                                                        @endif
                                                    </div>
                                                @endif
                                        ...
                                        @foreach($uncategorized as $cc)
                                            ...
                                            <div class="course-row d-flex justify-content-between align-items-center">
                                                <div>
                                                 ...
                                                 @if(in_array($cc->id,$educator_courses))
                                                        <span class="btn btn-in-completed">✓ Active Course</span> 
                                                    @elseif(in_array($cc->id,$pending_courses))
                                                            <span class="btn btn-in-progress">Pending Approval</span> 
                                                    @else
                                                    <form class="confirm-first enroll-form" action="{{ route('apply-course',$cc->id) }}" method="POST" id="enroll-form-{{ $cc->id }}">
